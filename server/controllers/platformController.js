...
exports.postToChannels = async (req, res) => {
  try {
    ...
    for (const [platformName, platformChannels] of Object.entries(
      channelsByPlatform,
    )) {
      try {
        const config = configs[platformName];
        ...
        for (const channel of platformChannels) {
          try {
            const postData = {
              ...
              media: mediaFile
                ? { file: mediaFile, mediaId: mediaInfo?.mediaId }
                : null,
            };
            ...
            console.error(
              `Error posting to ${platformName} channel ${channel.channelId}:`,
              error,
            );
            results.push({
              platform: platformName,
